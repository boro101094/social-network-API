...
const thoughtController = {
  ...
  getSingleThought(req, res) {
    Thought.findOne({ _id: req.params.thoughtId })
      .then((dbThoughtData) => {
        if (!dbThoughtData) {
          return res.status(404).json({ message: 'Error: Thought ID doesn\'t exist.' });
        }
        res.json(dbThoughtData);
      })
      .catch((err) => {
  ...
  createThought(req, res) {
    Thought.create(req.body)
      ...
      .then((dbUserData) => {
        if (!dbUserData) {
          return res.status(404).json({ message: 'Thought successfully created, but user ID doesn\'t exisit' });
        }
